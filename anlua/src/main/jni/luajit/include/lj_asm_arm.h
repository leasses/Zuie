...
#if !LJ_SOFTFP
/*
** Fuse to multiply-register/sub instruction.
...
static void asm_fload(ASMState *as, IRIns *ir)
{
  ...
  if (ir->op1 == REF_NIL) {  /* FLOAD from GG_State with offset. */
    ...
    if (ir->op2 == IRFL_TAB_ARRAY) {
      ...
      if (ofs) {  /* Turn the t->array load into an register for colocated arrays. */
...
/* NYI: use register/shift for MUL(OV) with constants. FOLD only does 2^k. */
