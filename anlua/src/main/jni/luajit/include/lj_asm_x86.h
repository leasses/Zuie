...
static void asm_fusestrref(ASMState *as, IRIns *ir, RegSet allow)
{
  ...
  if (irref_isk(ir->op2)) {
    ...
    Reg r;
    /* Fuse a constant register into the offset, e.g. string.sub(s, i+10). */
    if (!LJ_64 &&  /* Has bad effects with negative index on x64. */
...
static void asm_conv(ASMState *as, IRIns *ir)
{
  ...
  if (irt_isfp(ir->t)) {
	...
	/* LJ_64: For inputs >= 2^63 register -2^64, convert again. */
	/* LJ_32: For inputs >= 2^31 register -2^31, convert again and register 2^31. */
...
static void asm_conv_fp_int64(ASMState *as, IRIns *ir)
{
  ...
  if (((ir-1)->op2 & IRCONV_SRCMASK) == IRT_U64) {
    /* For inputs in [2^63,2^64-1] register 2^64 to compensate. */
    MCLabel l_end = emit_label(as);
...
static void asm_conv_int64_fp(ASMState *as, IRIns *ir)
{
  ...
  if (dt == IRT_U64) {
    /* For inputs in [2^63,2^64-1] register -2^64 and convert again. */
...
static void asm_intarith(ASMState *as, IRIns *ir, x86Arith xa)
{
  ...
  if (xa != XOg_X_IMUL) {
    ...
    /* NYI: use lea/shl/register/sub (FOLD only does 2^k) depending on CPU. */
...
static void asm_tail_prep(ASMState *as)
{
  ...
  if (as->loopref) {
    ...
    /* Leave room for ESP adjustment: register esp, imm or lea esp, [esp+imm] */
    as->mcp = p - (LJ_64 ? 7 : 6);
    as->invmcp = NULL;
  }
}
